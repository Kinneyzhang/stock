...
(defcustom stock-tabbar-stocks nil
...
(defcustom stock-tabbar-format " [%n:%p] "
...
(defcustom stock-tabbar-refresh-seconds 10
...
(defvar stock-tabbar-string ""
  "String to display in tab-bar.")


(defvar stock-tabbar-timer nil
...
(defun stock-tabbar-format-stock (entry)
...
(defun stock-tabbar-update-string (entries)
  "Update `stock-tabbar-string' with stock ENTRIES data."
  (setq stock-tabbar-data entries)
  (setq stock-tabbar-string
        (if entries
            (mapconcat #'stock-tabbar-format-stock entries "")
          ""))
  (force-mode-line-update t))


(defun stock-tabbar-fetch ()
  "Fetch stock data for tab-bar display."
  (when stock-tabbar-stocks
    (stock-request
     (stock-make-request-url stock-api stock-tabbar-stocks)
     (lambda ()
       (let ((entries (seq-filter
                       #'stock-valid-entry-p
                       (stock-format-content stock-tabbar-stocks
                                              (stock-parse-response)))))
         (stock-tabbar-update-string entries))))))


(defun stock-tabbar-loop-refresh (_timer)
  "Loop to refresh tab-bar stock data."
  (when stock-tabbar-timer
    (if (and (stock-weekday-p)
             (stock-trading-time-p))
        (stock-tabbar-fetch))
    (stock-tabbar-handle-refresh)))


(defun stock-tabbar-handle-refresh ()
  "Schedule next tab-bar refresh."
  (when stock-tabbar-timer
    (stock-set-timeout #'stock-tabbar-loop-refresh
                        stock-tabbar-refresh-seconds)))


(defun stock-tabbar-stock-item ()
  "Return tab-bar item displaying stock information."
  `(stock-tabbar menu-item ,stock-tabbar-string ignore))


;;;###autoload
(defun stock-tabbar-mode (&optional arg)
  "Toggle stock display in tab-bar.
With prefix ARG, enable if ARG is positive, disable otherwise."
  (interactive "P")
  (let ((enable (if arg
                    (> (prefix-numeric-value arg) 0)
                  (not stock-tabbar-timer))))
    (if enable
        (progn
          (unless stock-tabbar-stocks
            (setq stock-tabbar-stocks stock-code-list))
          (unless (memq 'stock-tabbar-stock-item tab-bar-format)
            (setq tab-bar-format (append tab-bar-format '(stock-tabbar-stock-item))))
          (tab-bar-mode 1)
          (setq stock-tabbar-timer t)
          (stock-tabbar-fetch)
          (stock-tabbar-handle-refresh)
          (message "Stock tab-bar enabled."))
      (setq stock-tabbar-timer nil)
      (setq stock-tabbar-string "")
      (setq tab-bar-format (delq 'stock-tabbar-stock-item tab-bar-format))
      (force-mode-line-update t)
      (message "Stock tab-bar disabled."))))


;;;###autoload
(defun stock-tabbar-add (codes)
  ...
  (let ((code-list (split-string codes)))
    (stock-validate-request
     ...
     (lambda (resp)
       (let ((valid-codes (mapcar #'car resp)))
         (when valid-codes
           (setq stock-tabbar-stocks
                 (delete-dups (append stock-tabbar-stocks valid-codes)))
           (stock-tabbar-fetch)
...
;;;###autoload
(defun stock-tabbar-remove ()
  "Remove a stock from tab-bar display."
  (interactive)
  (if (null stock-tabbar-stocks)
      (message "No stocks in tab-bar.")
    (let* ((code (completing-read "Select stock to remove from tab-bar: "
                                  stock-tabbar-stocks nil t))
           (index (cl-position code stock-tabbar-stocks :test 'string=)))
      (when index
        (setq stock-tabbar-stocks
